...
To combine DPoS and PoA for consensus, DeCloud Chain implement a novel consensus engine called Parlia that:
...
To achieve the cross-chain communication from Binance Chain to DeCloud Chain, need introduce a on-chain light client verification algorithm.
...
DCB will run on DeCloud Chain in the same way as ETH runs on Ethereum so that it remains as `native token` for BSC. This means, 
DCB will be used to:

1. pay `gas` to deploy or invoke Smart Contract on BSC
2. perform cross-chain operations, such as transfer token assets across DeCloud Chain and Binance Chain.
 ...
 * Start `geth` in fast sync mode (default, can be changed with the `--syncmode` flag),
   ...
   of the DeCloud Chain network, which is very CPU intensive.
...
DeCloud Chain network via your own programs and not manually through the console. To aid
